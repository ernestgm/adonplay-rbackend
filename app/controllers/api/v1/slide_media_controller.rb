    ...
    class SlideMediaController < ApplicationController
      ...
      def create
        ...
        if params[:audio].present?
          ...
          audio_filename = "#{SecureRandom.uuid}_#{audio_file.original_filename}"

          @slide = Slide.find(params[:slide_id])
          @business = @slide.business
          owner_id = @business.owner_id

          media_type = FileClassifier.classify(audio_file.original_filename)
          media = Media.new(media_type: media_type, owner_id: owner_id)

          # Create a temporary file
          temp_file_path = Rails.root.join('tmp', audio_filename)
          File.open(temp_file_path, 'wb') do |f|
            f.write(audio_file.read)
          end

          # Define the remote path for SFTP
          remote_path = "#{ENVIRONMENT}/medias/user_#{owner_id}/#{media_type.pluralize}/#{audio_filename}"
          ...
          begin
            ...
            # Save the media record
            if media.save
              @slide_media.audio_media_id = media.id
            else
              # Clean up temporary file
              File.delete(temp_file_path) if File.exist?(temp_file_path)
              # Return error
      ...
      def update
        ...
        if params[:file].present?
          ...
          begin
            ...
            # Save the media record
            if media.save
              permitted_params[:media_id] = media.id
            else
              # Clean up temporary file
              File.delete(temp_file_path) if File.exist?(temp_file_path)
              # Return error
        ...
        if params[:audio].present?
          ...
          begin
            ...
            # Save the media record
            if media.save
              permitted_params[:audio_media_id] = media.id
            else
              # Clean up temporary file
              File.delete(temp_file_path) if File.exist?(temp_file_path)
              # Return error
